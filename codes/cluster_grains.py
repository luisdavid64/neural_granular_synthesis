...
def encode_and_resynthesize(model, audio_path, output_path=None, use_cond=False):
    ...
    print(all_grains_2d.shape)
    # Save 2D embeddings
    # np.save(output_path.replace(".wav", "_grains_umap.npy"), all_grains_2d)
    # Save plot of 2D embeddings
    import matplotlib.pyplot as plt
    plt.figure(figsize=(10, 10))
    plt.scatter(all_grains_2d[:, 0], all_grains_2d[:, 1], s=2, alpha=0.6)
    plt.title("UMAP of Audio Grains")
